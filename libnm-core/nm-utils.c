...
_nm_utils_team_link_watcher_from_json (json_t *json_element)
{
	const char *j_key;
	json_t *j_val;
	char *name = NULL, *target_host = NULL, *source_host = NULL;
	...
	return nm_team_link_watcher_new (name, val1, val2, val3, target_host,
                           ...
                           const GValue *value)
{
	...
	if (G_VALUE_HOLDS_STRING (value))
		...
		if (nm_streq (key, "link_watch")) {
			...
			g_warning ("Composite value unsupported for team property: %s%s%s%s%s",
			           key,
			           key2 ? "." : "",
			           key2 ? : "",
			           key3 ? "." : "",
 ...
 **/
GVariant *
nm_utils_team_link_watchers_to_variant (GPtrArray *link_watchers)
{
	GVariantBuilder builder;
	int i;
 ...
 **/
GPtrArray *
nm_utils_team_link_watchers_from_variant (GVariant *value)
{
	...
	while (g_variant_iter_next (&iter, "@a{sv}", &watcher_var)) {
		...
		if (!g_variant_lookup (watcher_var, "name", "&s", &name)) {
			g_warning ("Ignoring link watcher: missing name");
			goto next;
		}
		...
		if nm_streq (name, NM_TEAM_LINK_WATCHER_ETHTOOL) {
			...
			if (!g_variant_lookup (watcher_var, "target-host", "&s", &target_host)) {
				g_warning ("Ignoring link watcher: missing target");
				goto next;
			}
			...
			if nm_streq (name, NM_TEAM_LINK_WATCHER_ARP_PING) {
				if (!g_variant_lookup (watcher_var, "source-host", "&s", &source_host)) {
					g_warning ("Ignoring link watcher: missing source");
					goto next;
				}
				if (g_variant_lookup (watcher_var, "validate-active", "b"))
					flags &= NM_TEAM_LINK_WATCHER_ARP_PING_FLAG_VALIDATE_ACTIVE;
				if (g_variant_lookup (watcher_var, "validate-inactive", "b"))
					flags &= NM_TEAM_LINK_WATCHER_ARP_PING_FLAG_VALIDATE_INACTIVE;
		...
		if (!watcher) {
			g_warning ("Ignoring link watcher: %s", error->message);
			g_clear_error (&error);
			goto next;
		}
